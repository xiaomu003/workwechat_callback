...
class Prpcrypt(object):
    ...
    #     """
    #     # 16位随机字符串添加到明文开头
    #     text = bytes(self.get_random_str(), encoding='utf8') + struct.pack("I", socket.htonl(len(text))) +  text.encode() + bytes(
    #         receiveid, encoding='utf8')
    ...
    #     try:
    #         ciphertext = cryptor.encrypt(bytes(text, encoding='utf8'))
    ...
    def encrypt(self, text, receiveid):
        ...
        # text = self.get_random_str() + binascii.b2a_hex(len_str).decode() + text + receiveid
        text = self.get_random_str() + len_str + text.encode() + receiveid
        # 使用自定义的填充方式对明文进行补位填充
        pkcs7 = PKCS7Encoder()
        text = pkcs7.encode(text)
        # 加密
...
#验证URL
    ...
    def VerifyURL(self, sMsgSignature, sTimeStamp, sNonce, sEchoStr):
        ...
        return ret,sReplyEchoStr.decode()
    ...
    def EncryptMsg(self, sReplyMsg, sNonce, timestamp = None):
        ...
        if ret != 0:
            return ret,None
        xmlParse = XMLParse()
        # print(xmlParse.generate(encrypt, signature, timestamp, sNonce))
